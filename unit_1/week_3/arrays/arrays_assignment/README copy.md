...
1. Write code such that given an array `['a', 'b', 'c', 'd', 'e']` and a `target = 'c'` it logs
`true` if the target is inside the array and false otherwise.
...
2. Write code such that noDupeList has all the same numbers as dupeFriendlyList, but has no more than one of each number.
...
<details>
  ...
  * First you will need to reuse the code written for bonus question 1. Solve it this way.
  * After search for an array method that could help you find out whether an element is already in an array?. Include both solutions.
</details>
